...
class BusinessServicer(business_pb2_grpc.BusinessServiceServicer):
    ...
    def GetBusiness(self, request, context):
        db = SessionLocal()
        try:
            row = cast(dict[str, Any] | None, get_business_by_id(db, request.business_id))
            if not row:
    ...
    def ListBusinesses(self, request, context):
        db = SessionLocal()
        try:
            rows = cast(list[dict[str, Any]], get_businesses(
                db,
...
def _to_proto(row: Mapping[str, Any]) -> business_pb2.BusinessResponse:
    return business_pb2.BusinessResponse(
        id=row.get("id", ""),
